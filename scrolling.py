...
hands = mp_hands(max_num_hands=1, min_detection_confidence=0.7)
...
def detecting_gesture(landmarks, handedness):
    fingers = []
    tips = [mp.hands.HandLandmark.INDEX_FINGER_TIP, mp_hands.HandLandmark.MIDDLE_FINGER_TIP, mp_hands.HandLandmark.RING_FINGER_TIP, mp_hands.HandLandmark.PINKY_FINGER_TIP]
    ...
    thumb2_tip = landmarks.landmark[mp.hands.HandLandmark.THUMB2_TIP]
    if (handedness == "Right" and thumb_tip.x > thumb2_tip.x) or (handedness == "Left" and thumb_tip.x < thumb2_tip.x):
        fingers.append(1)
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand, handedness_info in zip(results.multi_hand_landmarks, results.multi_handedness):
            ...
            gesture = detecting_gesture(hand, handedness)
